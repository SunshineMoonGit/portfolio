...
{#if prev || next}
  <nav class="flex justify-between gap-4 mt-12 pt-8 border-t border-border">
    {#if prev}
      <SurfaceCard href={`/notes/${prev.slug}`} class="flex flex-col gap-1.5 max-w-[48%] p-4 px-5 group">
    ...
    {/if}
    {#if next}
      <SurfaceCard href={`/notes/${next.slug}`} class="flex flex-col gap-1.5 max-w-[48%] p-4 px-5 text-right ml-auto group">
